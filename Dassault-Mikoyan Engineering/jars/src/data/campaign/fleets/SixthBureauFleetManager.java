...
public class SixthBureauFleetManager extends BaseCampaignEventListener implements EveryFrameScript {

	public static Logger log = Global.getLogger(SixthBureauFleetManager.class);
	...
	public SixthBureauFleetManager(MarketAPI market) {
		super(true);
		this.market = market;
	...
	public void advance(float amount) {
		...
		if (activePatrols.size() < maxPatrols) {
			...
			fleet.setFaction("dassault_mikoyan", true);
...
//					(type == PatrolType.COMBAT && (float) Math.random() > 0.5f)) {
			...
			log.info("Spawned Sixth Bureau patrol [" + fleet.getNameWithFaction() + "] from market " + market.getName());
		}
	}
